...
echo "    \"[보조 AI]한테 리뷰 요청\"             → 내 결론 작성 → bridge 전달 → 상대 리뷰 대기"
echo "    \"[보조 AI] 응답 확인\" / \"bridge 확인\"  → 상대 응답 회수 → reconcile → 다음 round/종료 판단"
echo "    원칙: 주 AI가 sequential coordinator, 병렬 독립결과를 따로 final로 내지 않음"
echo "    종료조건: Claude+Codex Confidence ≥90 + 결론 충족, 또는 round 5에서 사용자 결정"
...
echo "    \"[주 AI]한테 리뷰 요청\"                → 내 결론 작성 → bridge 전달 → 주 AI 회수 대기"
echo "    \"bridge 확인\" / \"[주 AI] 답변 확인\"     → 최신 항목 읽고 reconcile/다음 round/종료 판단"
echo "    원칙: 요청자가 응답을 회수해 합친 뒤 사용자에게 결론 공유"
echo "    종료조건: Claude+Codex Confidence ≥90 + 결론 충족, 또는 round 5에서 사용자 결정"
